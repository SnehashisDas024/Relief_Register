      ...
      <div className="max-w-full mx-auto px-4">
        <div className="flex items-center justify-between h-14">
          ...
          <div className="hidden lg:flex items-center gap-1">
            {navLinks().map((link, i) => (
              <Link key={i} to={link.to} className={`text-slate-300 hover:text-white px-3 py-2 rounded-lg text-sm no-underline transition-colors ${location.pathname === link.to ? "bg-white/10 text-white" : ""}`}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative">
              <button onClick={() => { setShowNotifs(!showNotifs); setShowUserMenu(false); }} className="text-slate-300 hover:text-white relative p-2" aria-label="Notifications">
              ...
              {showNotifs && (
                <div className="absolute right-0 mt-2 w-80 bg-white rounded-xl shadow-xl border border-sra-border z-50 fade-in-up">
                  <div className="p-3 border-b border-sra-border font-semibold text-sra-dark text-sm">Notifications</div>
                  {notifications.length > 0 ? notifications.map(n => (
                    <div key={n.id} className={`p-3 border-b border-sra-border text-sm hover:bg-slate-50 cursor-pointer ${!n.is_read ? "bg-blue-50" : ""}`}>
            ...
            {/* User Menu */}
            <div className="relative">
              <button onClick={() => { setShowUserMenu(!showUserMenu); setShowNotifs(false); }} className="flex items-center gap-2 text-slate-300 hover:text-white" aria-label="User menu">
              ...
              {showUserMenu && (
                <div className="absolute right-0 mt-2 w-56 bg-white rounded-xl shadow-xl border border-sra-border z-50 fade-in-up">
                  ...
                  <button onClick={() => { logout(); navigate("/login"); }} className="w-full text-left p-3 text-red-600 hover:bg-red-50 text-sm flex items-center gap-2 rounded-b-xl">
        ...
        {mobileOpen && (
          <div className="lg:hidden border-t border-white/10 pb-3 fade-in-up">
            {navLinks().map((link, i) => (
              <Link key={i} to={link.to} onClick={() => setMobileOpen(false)} className={`block text-slate-300 hover:text-white hover:bg-white/10 px-3 py-2.5 rounded-lg text-sm mt-1 no-underline ${location.pathname === link.to ? "bg-white/10 text-white" : ""}`}>
